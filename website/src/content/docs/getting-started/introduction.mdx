...
To properly understand how Astro Editor works with Astro projects, we need to look at 
...
![alt text](../../../assets/ae-intro-vscode-screenshot.png)

This is not a pleasent interface for writing content.
...
This same file is shown like this is Astro Editor...

![alt text](../../../assets/ai-intro-screenshot.png)
...
3. The frontmatter is instead shown in the right sidebar, and even though there's no `slug` or `description` in the file's frontmatter there are empty fileds for them in the sidebar because Astro Editor has read the schema and knows those fields are available for notes.
...
- The `components/mdx/` directory contains components like `Callout.astro`, which can be easily inserted into content using the [MDX Combonent builder]().

<Aside>
The paths for these directories are configurable in the [Preferences](/preferences/).
</Aside>
...
![alt text](../../../assets/pane-overview.png)

| Interface Area    | Purpose                                                                      |
| ----------------- | ---------------------------------------------------------------------------- |
| **Left Sidebar**  | Browse, sort and filter collections and files |
| **Main Editor**   | The main editor window                        |
| **Right Sidebar** | Frontmatter editor       |
...
![alt text](../../../assets/clean-editor-view.png)
