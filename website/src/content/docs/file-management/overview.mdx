...
Use `File > Open Project` to select your Astro project directory. Astro Editor will automatically scan for content collections defined in your `src/content/config.ts` or `src/content.config.ts` file.

**Directory Restrictions**: For security reasons, Astro Editor cannot open projects located in certain system directories. If you attempt to open a project in one of these locations, you'll see an error message asking you to choose a different location.
...
Use <Kbd mac="Cmd+N" windows="Ctrl+N" /> or the "New File" button to create a new file. The file is created in the currently active location:
