...
By default, files are sorted by publication date, newest first. This uses whichever date field is configured for the collection — the `date` field by default, which you can change in [preferences](/preferences/#field-mappings). Files without a date appear at the top of the list.
