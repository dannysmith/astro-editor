...
Commands are grouped under the headings below, in this order. Some commands only appear when they're relevant — file commands need an open file, IDE commands need a [configured IDE](/file-management/ide-integration/), and there's one "Open Collection" command per collection in your project.
...
Saving has no palette command — use <Kbd mac="Cmd+S" windows="Ctrl+S" /> (files also auto-save).
...
These need an [IDE command](/file-management/ide-integration/) to be configured.
